...
{{
    ...
    // ИСКЛЮЧАТЬ (false): Оплата, квитанции, личный кабинет, счетчики, справки, свет, газ, тепло, ГВС (горячая вода), контактные данные других отделов.

    "stats_categories": {{
        "refusal_deadline": true/false, // Оператор отказался назвать сроки завершения работ (не знает, нет информации).
...
2. is_relevant_hard ставишь true ТОЛЬКО если проблема касается ХВС (холодной воды) или канализации. Если звонят по оплате или счетчикам — ставь false.
3. location: если адреса нет, оставь поля пустыми строками "". 
            """
        )
