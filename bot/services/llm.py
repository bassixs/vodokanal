...
3. Отсрочка БЕЗ сроков: "Ждите", "Потерпите", "Позвоните позже", "Перезвоните завтра", "Приняли заявку", "Передадим заявку", "Передадим", "Передам", "Зафиксировал", "Записал", "Хорошо, передам", "Хорошо, передадим", "Ждём информацию", "Как поступит информация". КРИТЕРИЙ: Эти фразы - маркер ТОЛЬКО если оператор НЕ назвал конкретный срок. Примеры МАРКЕРОВ: "Хорошо, передадим." ❌ (нет срока), "Приняли заявку, ждите." ❌ (нет срока). Примеры НЕ МАРКЕРОВ: "Приняли заявку, бригада выедет в 15:00." ✅ (есть срок), "Передадим, в течение часа позвоним." ✅ (есть срок).
...
5. Перекладывание ответственности: "Сообщим мастеру", "Уже передали старшему", "Передам руководству", "Это к кому-то другому", "Обратитесь в..." (без конкретики), "Мы уже сообщали...", "Не к нам". КРИТЕРИЙ: Оператор перекладывает решение на других БЕЗ обязательства вернуться с ответом или БЕЗ указания, когда житель получит информацию.
6. Минимизация: "Это нормально", "Ничего страшного", "Вообще не проблема", "Все так живут".
7. Игнорирование вопроса: Оператор НЕ отвечает на прямой вопрос жителя, вместо этого дает общие фразы ("Угу", "Ладно") БЕЗ последующего ответа по существу. Длительное молчание (>10 сек) после важного вопроса. ВАЖНО: Фразы "Угу", "Ладно", "Хорошо" в контексте активного слушания (когда после них идёт конкретный ответ или действие) — это НОРМА, не маркер отказа.
8. Невозможность контакта: "Номер не будет записан", "Обратной связи не будет", "Вам не перезвонят".

ВАЖНО: НЕ считать маркерами отказа фразы вежливости и служебные переходы оператора:
- "Сейчас", "Одну секундочку", "Минуточку", "Одну минуту"
- "Я уточню", "Я посмотрю", "Сейчас проверю", "Уточняю"
- "Да-да", "Понятно", "Хорошо" (в контексте подтверждения приема информации)
Эти фразы — нормальная часть диалога, если после них следует конкретная информация или действие.
</СПРАВОЧНИК МАРКЕРОВ ОТКАЗА>
...
5. КРИТИЧНО: Фразы "передадим", "передам", "приняли заявку", "зафиксировал", "записал", "хорошо передадим" — это МАРКЕР ОТКАЗА (категория "Отсрочка БЕЗ сроков"), если оператор НЕ называет конкретный срок выполнения или обратной связи. Примеры: "Хорошо, передадим" → МАРКЕР ❌ | "Приняли заявку, бригада в 14:00" → НЕ маркер ✅

<ПРАВИЛА РАЗБИВКИ СПИКЕРОВ>
При форматировании cleaned_dialogue используй следующую логику для точного определения ролей:

**Признаки ОПЕРАТОРА:**
- Приветствия: "Здравствуйте", "Добрый день", "Слушаю", "Водоканал", "Диспетчерская"
- Запросы информации: "Назовите адрес", "Уточните...", "Какой дом?", "Ваш телефон?"
- Формальные фразы: "Сейчас посмотрю", "Принял заявку", "Передам информацию", "Зафиксировал"
- Ответы на вопросы жителя
- Первая фраза в диалоге (обычно оператор отвечает на звонок)

**Признаки ЖИТЕЛЯ:**
- Жалобы: "У нас нет воды", "Прорвало трубу", "Вода грязная", "Канализация течет"
- Вопросы: "Когда дадут воду?", "Как долго?", "Почему?", "А сроки?"
- Называет адрес: "Улица...", "Дом номер...", "Квартира..."
- Выражение недовольства: "Уже сутки", "Это безобразие", "Опять", "Каждый раз"
- Эмоциональные реакции: "Ужас", "Кошмар", "Что делать?"

**Контекстные правила:**
- Если фраза задает вопрос ЖИТЕЛЮ → это ОПЕРАТОР
- Если фраза отвечает на вопрос оператора → это ЖИТЕЛЬ
- Если фраза содержит проблему/жалобу → это ЖИТЕЛЬ
- Если фраза дает инструкции/объяснения → это ОПЕРАТОР
- Если сомневаешься → смотри на предыдущую фразу и логику диалога

**Формат вывода:**
```
Оператор: Здравствуйте, водоканал слушает.
Житель: Добрый день, у нас нет воды уже два часа.
Оператор: Назовите адрес, пожалуйста.
Житель: Улица Ленина, дом 5, квартира 12.
```

ВАЖНО: Используй ТОЛЬКО метки "Оператор:" и "Житель:", никаких других вариантов (Спикер 1, Диспетчер, и т.д.).
</ПРАВИЛА РАЗБИВКИ СПИКЕРОВ>
            """
        )
